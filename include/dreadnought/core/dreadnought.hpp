...
private:
  ...
  FORCE_INLINE void process_packet(const Packet &pkt) noexcept {
    ...
    // Log packet arrival using the already-captured start_ts
  ...
  FORCE_INLINE void update_order_book(const Packet &pkt) noexcept {
    ...
    logger_.log(LogEvent::BOOK_UPDATED, md_state_.sequence_num,
                md_state_.last_update_ts);
  }
  ...
  FORCE_INLINE void send_order() noexcept {
    ...
    order_pkt.header.type = PacketType::ORDER_NEW; // Use correct packet type
    ...
    logger_.log(LogEvent::ORDER_SENT, order_pkt.client_order_id,
                order_pkt.header.exchange_ts);
  }
